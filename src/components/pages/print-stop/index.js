...
const date = _.decodeURL(segments[4]);
const sabab = _.decodeURL(segments[5]);
console.log(date);
init();
async function init() {
  ...
  console.log(matt, annee);
  // get student data
  const student = await stopstudentsTable
    .where({
      s_matt: matt,
  ...
  document.querySelectorAll(".mostawa").forEach((element) => {
    element.innerHTML = `${student[0].s_niv} ${student[0].s_choaba} ${student[0].s_section}`;
  });
  document.querySelectorAll(".chatbDate").forEach((element) => {
    element.value = date;
  });
  document.querySelectorAll(".sabab").forEach((element) => {
    element.value = sabab;
  });
