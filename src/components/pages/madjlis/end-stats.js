...
const hPrimary = ` <tr>
                ...
                <th colspan="2" id="$one">&gt;=5</th>
                <th colspan="2" id="$two">0-2.99</th>
                <th colspan="2" id="$three">3-3.99</th>
                <th colspan="2" id="$four">4-4.99</th>
                <th colspan="2" id="$five">5-5.99</th>
                <th colspan="2" id="$six">6-6.99</th>
                <th colspan="2" id="$seven">7-7.99</th>
                <th colspan="2" id="$eight">8-10</th>
              </tr>
              <tr>
                <th>العدد</th>
                <th>%</th>
...
function buildTable(cols, d, helperData) {
  const niv = cols.niv;
  ...
  for (let i = 0; i < d.length; i++) {
    ...
    trs += `<td>${add_ijaza(Number(dr.pop()))}</td></tr>`;
...
function calc(table, moyen) {
  ...
  for (let index = 5; index < rows[0].cells.length - 1; index++) {
    ...
    let two = _.isPrimary() ? 4 : 10;
    let three = _.isPrimary() ? 5 : 11;
    let four = _.isPrimary() ? 6 : 12;
    let five = _.isPrimary() ? 7 : 13;
    ...
    let less_eigth = aray.filter((item) => item < seven && item >= six);
    const big_sexteen = aray.filter((item) => item >= seven);
    less_eigth = _.isPrimary() ? big_sexteen : less_eigth;
    var note_array = {
      less_height: {
...
function four(cols, d, helperData) {
  ...
  $mawayHead.innerHTML = _.isPrimary() ? hPrimary : hOther;
  let tr = "";
...
function five(c, d) {
  ...
  var { new_one, new_note } = calc(table, Maxmoy);
  let ar = _.isPrimary() ? a_no.slice(0, 7) : a_no;
  let ara1 = _.isPrimary() ? a_no_tp : a_no_t;
  for (let index = 0; index < ar.length; index++) {
    add_row2(new_note, ar[index], ara1[index], "s-note-tabl");
  }
}
